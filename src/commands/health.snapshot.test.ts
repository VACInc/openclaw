...
describe("collectGatewayHealthSnapshot", () => {
  ...
  it.each([
    ...
    const plugin: HealthTestPlugin = {
      ...
      config: {
        ...
        isLinked: () => (phase === "isLinked" ? hang(true) : true),
      },
      status: {
    ...
    const result = snap.channels[pluginId]?.accounts?.default;
  ...
  it("keeps timed-out account work inside the five-account capacity", async () => {
    ...
    let started = 0;
    const plugin: HealthTestPlugin = {
      ...createChannelTestPluginBase({ id: "capacity-test", label: "Capacity test" }),
      config: {
    ...
    await vi.advanceTimersByTimeAsync(100);
    const snap = await run;
    const accounts = snap.channels[plugin.id]?.accounts;
