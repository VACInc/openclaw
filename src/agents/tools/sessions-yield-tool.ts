...
const SessionsYieldToolSchema = Type.Object({
  message: Type.Optional(
    Type.String({
      description: "Waiting status to send now; the subagent result arrives in a later message.",
    }),
  ),
});
...
export function createSessionsYieldTool(opts?: {
  ...
  return {
    ...
    description:
      "End turn after subagent spawn; results arrive later. Set message to send a waiting status now.",
