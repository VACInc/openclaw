...
export function describeSessionsSendTool(): string {
  return [
    ...
    'Thread chats rejected: the parent channel is a different session and cannot resume thread-owned work. For authorized continuation, wake the exact owning session through the available scheduler; read its result history before retrying work or reporting completion. A wake acknowledgement only confirms scheduling. Missing configured-agent main created. Waits for reply when available; status "no_reply" is terminal, so do not wait for an announcement.',
