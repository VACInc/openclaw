...
describe("startup recovery admission", () => {
  ...
  function makePendingFinalDelivery(): NonNullable<InternalSessionEntry["pendingFinalDelivery"]> {
    return {
      kind: "replayable",
  ...
  function observeScheduledRecoveryAdmissions(expectedTargets = 1) {
    const startupSettled = createDeferred();
    const targetRequested = createDeferred();
    const targetsSettled = createDeferred();
      ...
      .mockImplementation(
        async <T>(run: () => Promise<T>, origin?: string, signal?: AbortSignal) => {
          ...
          try {
            ...
            if (origin === "main-session:target-recovery" && ++settledTargets === expectedTargets) {
              targetsSettled.resolve();
            }
          }
        },
      );
    return {
      startupSettled: startupSettled.promise,
      targetRequested: targetRequested.promise,
      targetsSettled: targetsSettled.promise,
  ...
  it("tombstones when the final startup retry consumes the last charge", async () => {
    ...
    const admissions = observeScheduledRecoveryAdmissions();
    const recovery = scheduleRestartAbortedMainSessionRecovery({
    ...
    try {
      // Durable reconciliation, not a wall-clock polling budget, owns completion.
      await admissions.targetsSettled;
      expect(admissions.targetRequests()).toBe(1);
    ...
    await withEnvAsync({ OPENCLAW_STATE_DIR: tmpDir }, async () => {
      ...
      for (const [index, directory] of ["ops", " ops "].entries()) {
        const fixture = await makeMainSessionFixture({
          ...
          mainRestartRecovery: { cycleId: `cycle-ops-${index}`, revision: 1, chargedAttempts: 2 },
          pendingFinalDelivery: makePendingFinalDelivery(),
        });
      ...
      const admissions = observeScheduledRecoveryAdmissions(2);
      const recovery = scheduleRestartAbortedMainSessionRecovery({
      ...
      try {
        await admissions.targetsSettled;
        expect(admissions.targetRequests()).toBe(2);
  ...
  it("stops exhaustion reconciliation while its Gateway admission is suspended", async () => {
    ...
    try {
      await admissions.startupSettled;
      // Observe the request without waiting for its intentionally suspended admission.
      await admissions.targetRequested;
