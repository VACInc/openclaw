...
describe("runWithModelFallback + runEmbeddedAgent failover behavior", () => {
  ...
  it("falls back across providers after overloaded primary failure without cooling auth", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
  ...
  it("retries provider overload across turns without cooling auth", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
      mockPrimaryOverloadedThenFallbackSuccess();
  ...
  it("classifies bare service-unavailable failures as overload without cooling auth", async () => {
    await withAgentWorkspace(async ({ agentDir, workspaceDir }) => {
      await writeAuthStore(agentDir);
