...
function isSuppressedModel(provider?: string, id?: string): boolean {
  ...
  return provider === "azure-openai-responses" && modelId === "gpt-5.3-codex-spark";
}
...
describe("loadModelCatalog", () => {
  ...
  it("keeps direct OpenAI and Codex OAuth Spark while filtering stale Azure rows", async () => {
    mockPiDiscoveryModels([
      {
        id: "gpt-5.3-codex-spark",
        provider: "openai",
    ...
    expect(requireCatalogEntry(result, "openai", "gpt-5.3-codex-spark")).toMatchObject({
      input: ["text", "image"],
      contextWindow: 128000,
    });
