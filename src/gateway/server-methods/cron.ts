...
class CronUpdatePreconditionRejectedError extends Error {
  constructor(readonly rejection: unknown) {
    super(formatErrorMessage(rejection), { cause: rejection });
  }
}
...
async function assertValidCronUpdatePatch(params: {
  ...
  applyJobPatch(nextJob, params.patch, {
    ...
    scheduleValidationNowMs: params.nowMs,
...
export const cronHandlers: GatewayRequestHandlers = {
  ...
  "cron.update": async ({ params, respond, context, client }) => {
    ...
    try {
      job = await context.cron.updateWithPrecondition(
        ...
        async (lockedJob, nowMs) => {
          try {
            ...
            throw new CronUpdatePreconditionRejectedError(err);
          }
        },
        cronPatchTouchesToolRuntime(patch)
          ? {
      ...
      if (err instanceof CronUpdatePreconditionRejectedError) {
        respondCronFailedNoEffect(
          respond,
          errorShape(
            ErrorCodes.INVALID_REQUEST,
            `invalid cron.update params: ${formatErrorMessage(err.rejection)}`,
          ),
        );
        return;
      }
      // The grant producer throws this exact typed 403 before the service snapshot/persist boundary.
      // Other errors remain uncertain because they may follow a durable commit.
      if (
        isRecord(err) &&
        err instanceof TypeError &&
        err.name === "CronCreatorAuthorityExpiredError" &&
        err.status === 403
      ) {
        respondCronFailedNoEffect(
          respond,
