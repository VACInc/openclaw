...
Large stable content (including **Project Context**) stays above the internal prompt cache boundary. Volatile per-turn sections (Control UI embed guidance, **Messaging**, **Collapsible Details**, **Voice**, **Group Chat Context**, **Reactions**, **Heartbeats**, **Runtime**) are appended below that boundary so local backends with prefix caches can reuse the stable workspace prefix across channel turns. Claude CLI has no system-level cache breakpoint, so it sends that stable prefix as the CLI system prompt and puts the volatile suffix on the user turn. Tool descriptions should avoid embedding current channel names when the accepted schema already carries that runtime detail.
