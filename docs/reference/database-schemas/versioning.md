...
### Raw transcript navigation

The [accepted navigation design](https://github.com/openclaw/openclaw/pull/147793#issuecomment-5670005343) adds two bare nullable `TEXT` columns at agent schema 20: `transcript_events.navigation_json` and `transcript_rewrite_watermarks.navigation_generation`. Raw event bytes remain canonical. The first column holds versioned, presence-aware tree/reset/role facts, encoded as JSON text so NUL and lone-surrogate identities survive exactly. Physical sequence numbers remain only in the row key. A partial index finds missing headers; the existing generation owner certifies their coverage. There is no new table, scheduler, trigger, retention rule, or numeric schema bump.

Identity strings retain their exact code units. Enum-like fields and timestamp replay eligibility use bounded facts from the existing tree/replay owners; unsupported values remain present but invalid instead of copying arbitrary payloads into the header.

Current writers maintain headers with raw inserts and exact rewrites. Exact retained copies preserve headers; SQL-reparented opaque rows invalidate them until repair reads the actual result. Canonical repair and cold restoration reconstruct headers without changing raw bytes or the cold archive format. Deletion removes headers with their raw rows.

Writable startup adds the columns and index without parsing history. The existing transcript reconciliation worker backfills bounded chunks off-thread and commits them only after checking the raw generation, mutation watermark, sequence boundary, and exact source BLOB bytes. Metadata-only gaps do not rebuild healthy full-text indexes. Metadata repair never rotates the raw generation or changes session activity. A missing header or stale generation makes optional memory capture unavailable until catch-up finishes; an unrelated dirty active index does not. Reset preparation joins the same repair lifecycle before its guarded write rather than synchronously parsing old history on the request thread; a first reset may still wait for catch-up. Chunk byte/row targets allow one oversized raw value, so its worker and IPC memory cost is not constant. Incognito source framing also starts with one complete native row copy. Raw history remains readable.

Malformed raw JSON gets a terminal poison header rather than an endlessly retried missing row. Poison, malformed metadata, or an unsupported payload version makes navigation unavailable; canonical transcript repair is required for malformed raw data. Non-record JSON has no navigation facts and is represented as such, without copying its opaque value into the header.

The additive columns and index remain compatible with the `v2026.9.4` schema validator. Its writer ignores them: append leaves missing headers, while destructive rewrites rotate the existing raw generation and invalidate the retained certificate. This does not make a full binary rollback to `v2026.9.4` supported: that release supports agent schema 19, while the existing cold-storage contract already requires schema 20. Keep the current version guard and use a build that supports the existing schema. Any rollback procedure must preserve current canonical history; do not lower version markers or discard newer history by blindly restoring an older backup. Same-version writers that omit the headers leave them missing; re-upgrade catches up through the same worker. Direct SQL edits that bypass the transcript writer must also invalidate coverage or rotate the generation; the metadata is not a detector for out-of-contract raw database edits. Read-only access to a pre-migration schema reports unavailable navigation and does not add columns.
