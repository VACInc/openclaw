...
Set `tools.exec.reviewer.serviceTier` to `"priority"` to request Fast processing on supported OpenAI Responses and ChatGPT/OAuth routes, or `"default"` for standard processing. For example, `reviewer: { model: "openai/gpt-5.6-terra", thinking: "low", serviceTier: "priority" }` requests both low reasoning effort and priority processing. Omit `serviceTier` to preserve provider defaults. This setting is independent of the main agent's Fast mode and is also available per agent. Priority processing may cost more and remains subject to provider/model availability; other providers may ignore the setting.
