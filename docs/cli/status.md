...
| `--timeout <ms>`        | Timeout for status operations, in milliseconds.                                                                 |
...
older Gateways use a 10-second response timeout. An explicit timeout applies to
Gateway requests, provider usage, security audits, heartbeat and service probes,
and full-report collection; the Gateway keeps its own per-account probe deadline.
