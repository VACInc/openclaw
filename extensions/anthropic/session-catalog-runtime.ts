...
function claudeBindingHostId(entry: {
  execHost?: string;
  execNode?: string;
...
function boundClaudeSource(
  pluginId: string,
  entry: Parameters<typeof adoptedClaudeSource>[1],
): { hostId: string; threadId: string } | undefined {
...
export function listManagedClaudeSessionCandidates(
  ...
  for (const { entry } of listSessionCatalogEntries({
    // Migration is catalog-wide even when the current caller is scoped to one
    // explicitly owned agent. Do not commit the one-time marker from a partial scan.
