  ...
  it.each([
    ...
    await withOpenClawTestState({ prefix: "notice-http-" }, async (state) => {
      ...
      const productionPredicate =
        mode === "production allowed" || mode === "runtime policy revoked";
      ...
      try {
        ...
        if (mode === "runtime policy revoked") {
          ...
          expect(isRestartRecoveryDeliveryCurrent({ ...scope, cfg })).toBe(true);
          expect(isRestartRecoveryDeliveryCurrent({ ...scope, cfg: currentCfg })).toBe(false);
        }
...
describe("recovery typing final transport fence", () => {
  it.each([
    ...
    await withOpenClawTestState({ prefix: "typing-http-" }, async (state) => {
      ...
      const guardedTyping = telegramPlugin.heartbeat?.sendTypingV2;
      if (!guardedTyping) {
        throw new Error("Missing guarded Telegram typing hook");
      }
      const trackedTyping: NonNullable<ChannelHeartbeatAdapter["sendTypingV2"]> = async (
        params,
      ) => {
        started.resolve();
        try {
        ...
        createTestRegistry([
          {
            ...
            plugin: {
              ...telegramPlugin,
              heartbeat: { ...telegramPlugin.heartbeat, sendTypingV2: trackedTyping },
            },
          },
        ]),
        null,
